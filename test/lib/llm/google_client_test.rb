...
class LLM::GoogleClientTest < ActiveSupport::TestCase
  ...
  test "initializes with predefined model keys" do
    client = LLM::GoogleClient.new(model: LLM::GoogleClient::MODELS["flash-2-0"])
    assert_equal "gemini-2.0-flash", client.model

    client = LLM::GoogleClient.new(model: LLM::GoogleClient::MODELS["flash-2-5"])
    assert_equal "gemini-2.5-flash-preview-04-17", client.model
  end
  ...
  test "environment variable is used for API key" do
    ...
    mock_response.stubs(:body).returns('{"candidates":[{"content":{"parts":[{"text":"Test"}]}}],"modelVersion":"gemini-2.0-flash"}')
  ...
  test "send_request formats request and returns LLMResponse" do
    ...
    @http_mock.expects(:request).with do |request|
      ...
      assert_match /\/v1beta\/models\/gemini-2.0-flash:generateContent/, request.path
    ...
    assert_equal "gemini-2.0-flash", response.model
  end
